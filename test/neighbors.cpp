...
namespace starkware {
    namespace {
        ...
        TEST(Neighbors, Iterator) {
            ...
            const std::array<std::pair<int64_t, uint64_t>, 5> mask = {{{0, 0}, {0, 1}, {1, 6}, {2, 0}, {2, 3}}};
            ...
            const Neighbors neighbors(mask, std::vector<gsl::span<const BaseFieldElement>>(trace.begin(), trace.end()),
                                      std::vector<gsl::span<const ExtensionFieldElement>>(composition_trace.begin(),
                                                                                          composition_trace.end()));
        ...
        TEST(Neighbors, InvalidMask) {
            ...
            const std::array<std::pair<int64_t, uint64_t>, 5> mask = {{{0, 0}, {0, 1}, {1, 2}, {2, 0}, {2, 3}}};
            std::vector<std::vector<BaseFieldElement>> trace;
            Prng prng;
            ...
            EXPECT_ASSERT(
                Neighbors(mask, std::vector<gsl::span<const BaseFieldElement>>(trace.begin(), trace.end()), {}),
