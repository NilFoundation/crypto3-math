#include <nil/crypto3/math/polynomial/composition/breaker.hpp>
    ...
    namespace {
        ...
        /*
          ...
          4. Use PolynomialBreak::EvalFromSamples() to compute f(x_j) and compare to the result from the
             previous step.
        */
        void TestPolynomialBreak(const size_t log_domain, const size_t log_breaks, const size_t n_check_points) {
            Prng prng;
            ...
            // Step 2. Break.
            std::vector<ExtensionFieldElement> break_storage(evaluation.size(), ExtensionFieldElement::Uninitialized());
            ...
            std::vector<ExtensionFieldElement> expected_results(n_check_points, ExtensionFieldElement::Uninitialized());
            lde_manager.EvalAtPoints<ExtensionFieldElement>(0, points, expected_results);

            // Prepare to compute h_i(x_j^(2^log_breaks)).
            LdeManager<ExtensionFieldElement> broken_lde_manager(broken_coset, /*eval_in_natural_order=*/false);
            ...
            for (size_t i = 0; i < n_breaks; ++i) {
                ...
                broken_lde_manager.EvalAtPoints<ExtensionFieldElement>(i, points_transformed, sub_results.back());
            }
